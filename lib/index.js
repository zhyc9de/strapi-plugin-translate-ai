  ...
  init(providerOptions = {}) {
    ...
    return {
      ...
      async translate({ text, priority, sourceLocale, targetLocale, format }) {
        ...
        const results = []
        for (const chunkTexts of chunks) {
          try {
            console.log(
              "🚀 ~ Processing chunk:",
              chunkTexts.length,
              "texts, total characters:",
              chunkTexts.reduce((acc, str) => acc + str.length, 0),
            )
            ...
            results.push(chunkTexts.map(() => ""))
          }
        }
        const combinedResults = reduceFunction(results)
